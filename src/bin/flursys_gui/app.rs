use flursys::app::{JobProgress, WorkbenchJobEvent, WorkbenchJobExecutor};
...
use flursys::{
    ...
    IncompressibleBoundaryCondition, IncompressibleSolveError, LegacyProjectInspection,
...
struct AppState {
    ...
    meshing: bool,
    solving: bool,
    workbench_jobs: WorkbenchJobExecutor,
...
impl AppState {
    ...
    fn new() -> Self {
        ...
        Self {
            ...
            meshing: false,
            solving: false,
            workbench_jobs: WorkbenchJobExecutor::new(),
            gmsh_probe_rx: None,
    ...
    fn drain_workbench_jobs(&mut self) -> bool {
        ...
        for event in self.workbench_jobs.drain() {
            match event {
                WorkbenchJobEvent::MeshProgress(JobProgress::Started) => {
                    self.log("Gmsh worker started.");
                    changed = true;
                }
                WorkbenchJobEvent::MeshProgress(JobProgress::Cancelled) => {
                    self.meshing = false;
                    self.log("Gmsh generation cancelled.");
                    changed = true;
                }
                WorkbenchJobEvent::MeshCompleted(Ok(generated)) => {
                    self.meshing = false;
                    let report = format!(
                ...
                WorkbenchJobEvent::MeshCompleted(Err(error)) => {
                    self.meshing = false;
                    if matches!(error, flursys::MeshingError::Cancelled) {
                        self.log("Gmsh generation cancelled.");
                    } else {
                ...
                WorkbenchJobEvent::SolveProgress(JobProgress::Started) => {
                    self.log("Solve worker started.");
                    changed = true;
                }
                WorkbenchJobEvent::SolveProgress(JobProgress::Iteration {
                    current,
                    max,
                    residual,
                }) => {
                    if current == 1 || current == max || current % 10 == 0 {
                        self.log(format!(
                            "Solve iteration {current}/{max}, continuity {:.3e}.",
                            residual.unwrap_or(f64::NAN)
                        ));
                        changed = true;
                    }
                }
                WorkbenchJobEvent::SolveProgress(JobProgress::Cancelled) => {
                    self.solving = false;
                    self.log("Solve cancellation requested.");
                    changed = true;
                }
                WorkbenchJobEvent::SolveCompleted(outcome) => {
                    self.solving = false;
                    match &outcome {
                        ...
                        Err(flursys::IncompressibleSolveError::Cancelled) => self.log("Workbench solve cancelled."),
...
/// Initializes the application controller, binds the Slint surface, and enters
...
fn bind_callbacks(ui: &MainWindow, state: &Rc<RefCell<AppState>>) {
    ...
    ui.on_generate_mesh_wb(move || {
        ...
        match state.workbench.mesh_generation_inputs() {
            Ok((export, options)) => match state.workbench_jobs.start_mesh(export, options) {
                Ok(()) => {
                    state.meshing = true;
                    state.log(format!(
    ...
    ui.on_run_workbench(move || {
        ...
        match state.workbench_jobs.start_solve(case) {
            Ok(()) => {
                state.solving = true;
                state.log(format!(
            ...
            Err(error) => {
                state.workbench.complete_solve(Err(IncompressibleSolveError::Cancelled));
                state.log(format!("Cannot start solve: {error}"));
            }
        }
        rebuild_tree_rows(&mut state);
        refresh_ui(&ui, &state);
    ...
    let weak_ui = ui.as_weak();
    let cancel_job_state = state.clone();
    ui.on_cancel_workbench_job(move || {
        let Some(ui) = weak_ui.upgrade() else {
            return;
        };
        let mut state = cancel_job_state.borrow_mut();
        if state.workbench_jobs.cancel() {
            state.log("Cancellation requested for the active workbench job.");
        } else {
            state.log("There is no active workbench job to cancel.");
        }
        refresh_ui(&ui, &state);
    });
